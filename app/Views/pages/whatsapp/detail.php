    ...
    <div class="rounded-3 shadow-sm mb-4" style="background: linear-gradient(rgba(0,184,241,0.9), rgba(0,107,148,0.9)), url('https://images.unsplash.com/photo-1593697821034-73af01208535?auto=format&fit=crop&w=1350&q=80'); background-size: cover; background-position: center;">
        <div class="d-flex justify-content-between align-items-center p-4 text-white">
            <div>
                ...
                <p class="text-white-70 small mb-0">Kelola data WhatsApp prospek untuk pemasaran</p>
            </div>
            ...
            <div class="d-flex gap-2">
                ...
                <button type="button" class="btn btn-light text-info px-4 py-2 fs-6 d-flex align-items-center gap-2" id="btnAddWhatsapp" data-id-prospek="<?= $prospek['id_prospek'] ?>">
                    <i class="fas fa-plus-circle me-2"></i>
                    <span class="d-none d-sm-inline">Tambah WhatsApp</span>
                </button>
            </div>
        </div>
    </div>
        ...
        <div class="datatable-wrapper">
            <div class="table-responsive">
                <div class="table-responsive-wrapper">
                    <table id="whatsappTable" class="table align-middle">
                        ...
                        <tbody>
                            <?php if (!empty($whatsapp_history)): ?>
                                <?php foreach ($whatsapp_history as $index => $row): ?>
                                    <tr>
                                            ...
                                            <div class="d-flex align-items-center py-2">
                                                <div class="flex-grow-1 min-width-0">
                                                    ...
                                                    <small class="text-muted">
                                                        <i class="fab fa-whatsapp"></i> <?= $row['no_hp'] ?? 'Tidak ada no. hp' ?>
                                                    </small>
                                                </div>
                                            </div>
                                        </td>
        ...
        <div class="modal-content border-0 shadow-lg">
            ...
            <form id="editWhatsappForm">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="edit_pesan" class="form-label fw-semibold">Pesan WhatsApp <span class="text-danger">*</span></label>
        ...
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-info text-white">
                <h5 class="modal-title fw-semibold" id="messageModalLabel">
                    <i class="fas fa-envelope-open-text me-2"></i> Detail Pesan WhatsApp
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
...
<script>
    $(document).ready(function() {
        var table = $('#whatsappTable').DataTable({
            ...
            language: {
                decimal: "",
                emptyTable: "Tidak ada data WhatsApp yang tersedia",
            ...
            initComplete: function() {
                ...
                $('#whatsappTable_length').appendTo('#custom-length');
                $('#whatsappTable_filter').appendTo('#custom-search');
        ...
        // --- FUNGSI UNTUK TAMBAH MULTIPLE WHATSAPP ---
                            ...
                            html += `
                                <div class="form-check mb-2">
                                    ...
                                    <label class="form-check-label" for="company_add_${company.id_detail_prospek}">
                                        ...
                                        <small class="text-muted"><i class="fab fa-whatsapp"></i> ${company.no_hp || 'Tidak ada no. hp'}</small>
                                    </label>
                                </div>`;
                        });
                    } else {
                        html = '<div class="text-center text-muted p-3">Tidak ada perusahaan baru dengan No. HP yang valid di prospek ini.</div>';
                    }
                    container.html(html);
                    updateSelectedCount();
                },
        ...
        $(document).on('change', '#addMultipleWhatsappsModal .company-checkbox', updateSelectedCount);

        $('#addMultipleWhatsappsModal #selectAllCompanies').click(function() {
            $('#addMultipleWhatsappsModal .company-checkbox').prop('checked', true);
            updateSelectedCount();
        });

        $('#addMultipleWhatsappsModal #unselectAllCompanies').click(function() {
            $('#addMultipleWhatsappsModal .company-checkbox').prop('checked', false);
            updateSelectedCount();
        });

        function updateSelectedCount() {
            const count = $('#addMultipleWhatsappsModal .company-checkbox:checked').length;
        ...
        $('#addMultipleWhatsappsForm').submit(function(e) {
            ...
            const selectedCompanies = $('#addMultipleWhatsappsModal .company-checkbox:checked').map(function() {
                return this.value;
            }).get();
            ...
            $.ajax({
                ...
                dataType: 'json',
                beforeSend: function() {
                    $('#btnSaveMultipleWhatsapps').prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-2"></span>Menyimpan...');
                },
                success: function(response) {
                    if (response.success) {
        ...
        $(document).on('click', '.btn-edit-whatsapp', function(e) {
            e.preventDefault();
